...
"Please modify the keyword for searching target energy value in line 42"
...
def extract_structure_and_energy(log_file):
    ...
    linenumber = [ i for i,line in enumerate(log_content) if re.search("Input orientation:",line)][-1] 
    structure_start = linenumber + 5  # Start from the 5th line after "Input orientation:"
    ...
    with open(output_filename, 'w') as output_file, open("resub.sh",'w') as ff:
        for log_file in log_files:
            ...
            if structure is not None and energy is not None:
                ...
                output_file.write(f"{energy:.8f} #{log_file.split('.')[0]}\n")
